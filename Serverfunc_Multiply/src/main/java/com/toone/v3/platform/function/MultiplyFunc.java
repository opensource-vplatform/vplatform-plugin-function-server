 ...
 * 乘法<br>
 * <br>
 * 代码示例:ServerMultiply(arg1,arg2,...,argN,argN+1) 返回值为数字类型。<br>
 * 参数1--被乘数(数字类型)；<br>
 * 参数2--乘数(数字类型)；<br>
 * 参数N--乘数(数字类型)；<br>
...
//                    throw new ServerFuncException("函数【" + funcCode + "】的第" + (i+1) + "个参数不能为空");
                    ...
                    result = result.multiply(tmpNum.multiply(mulValue));
            }
    ...
    private static BigDecimal strBigDecimal(String val) {
        ...
        try {
            ...
            throw new ServerFuncException("ServerMultiply函数参数不正确，要求参数为数值类型，实际参数为" + val);
        }
    }
